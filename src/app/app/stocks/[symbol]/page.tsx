...
import { getReplayQuote, getReplaySeries } from "@/lib/market/replay-provider";
...
export default async function StockDetailPage({ params }: PageProps<"/app/stocks/[symbol]">) {
  ...
  const points = getReplaySeries(symbol, 30);
  return <>
    ...
    <div className="trade-grid">
      <section className="card" style={{ padding: "1.3rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "end" }}><div><span className="eyebrow">Replay price</span><strong style={{ display: "block", fontSize: "2.6rem", letterSpacing: "-.05em", marginTop: ".35rem" }}>{formatMoney(quote.price)}</strong><span className={quote.change >= 0 ? "positive" : "negative"} style={{ fontWeight: 850 }}>{formatMoney(quote.change)} · {formatPercent(quote.changePercent)} today</span></div><span className="status-pill">Market {quote.marketState}</span></div>
        <StockChart points={points} />
        <div className="info-box"><CircleAlert size={16} style={{ display: "inline", marginRight: 6 }} />Deterministic replay series for product development. It is not a live or historical licensed quote.</div>
      </section>
