...
export default async function TradePage({ params }: PageProps<"/app/trade/[symbol]">) {
  ...
  const { profile } = await syncAlpacaInstrument(asset, quote.price);
  const position = await getInstrumentPositionDTO(symbol);
  ...
  return <>
    ...
    <div className="page-title"><div><span className="eyebrow">Simulated order</span><h1>Trade {profile.name}</h1><p className="muted" style={{ margin: ".6rem 0 0" }}>{symbol} · {formatMoney(quote.price)} · <span className={quote.change >= 0 ? "positive" : "negative"}>{formatPercent(quote.changePercent)}</span></p></div></div>
